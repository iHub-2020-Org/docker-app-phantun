...
function updateLanguage(lang) {
    ...
    setTimeout(() => {
        if (typeof app !== 'undefined') {
            ...
            if (app.lastStatus) app.updateTunnelStatus();
        }
    }, 10);
}
...
const app = {
    ...
    updateTunnelStatus() {
        ...
        const clients = this.lastConfig?.clients || [];
        const servers = this.lastConfig?.servers || [];
        const allItems = [...clients.map(c => ({ ...c, _type: 'client' })), ...servers.map(s => ({ ...s, _type: 'server' }))];
        ...
        tbody.innerHTML = allItems.map(item => {
            ...
            const statusLabel = isRunning ? t('status.running') : t('status.stopped');
            const statusClass = isRunning ? 'running' : 'stopped';

            // Strict Config Display (Local: Bind IP:Port, Remote: Server IP:Port)
            const localDisplay = `${item.local_addr || '0.0.0.0'}:${item.local_port}`;
            const remoteDisplay = `${item.remote_addr || '...'}:${item.remote_port}`;

            return `
        <tr>
            <td>${this.escapeHtml(item.alias || item.id.substring(0, 8))}</td>
            <td>${item._type === 'client' ? t('mode.client') : t('mode.server')}</td>
            <td><span class="status-badge ${statusClass}">${statusLabel}</span></td>
            <td>${this.escapeHtml(localDisplay)}</td>
            <td>${this.escapeHtml(remoteDisplay)}</td>
        </tr>
    `;
        }).join('');
    },
    ...
    renderTopology(clients, servers) {
        ...
        const createSvgRow = (item, type, index) => {
            ...
            // Fetch Process Info (Only for status check, NOT for labels)
            ...
            const y = 40;
            const x1 = baseWidth * 0.10;
            const x2 = baseWidth * 0.35;
            const x3 = baseWidth * 0.65;
            const x4 = baseWidth * 0.90;
